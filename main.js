...
const attachBtn = (name, link) => {
  ...
  anchor.href = link;
  if (removeBtn.classList.contains("click")) eraseBtn.classList.add("display");
...
removeBtn.addEventListener("click", () => {
  for (let i = 0; i < data.size; i++) erasers[i].classList.toggle("display");
  removeBtn.classList.toggle("click");
});

if (data.size) for (const [name, link] of data) attachBtn(name, link);

importBtn.addEventListener("change", async () => {
  const imported_data = JSON.parse(await importBtn.files[0].text());
  for (const [name, link] of imported_data) update(name, link);
});
