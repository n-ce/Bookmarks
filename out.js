(() => {
  ...
  var erasers = document.getElementsByTagName("p");
  var anchors = document.getElementsByTagName("a");
  var saveData = (d) => localStorage.setItem("data", JSON.stringify([...d]));
  var removeBtn = document.getElementById("removeBtn");
  var updateBtn = document.getElementById("updateBtn");
  var importBtn = document.getElementById("upload");
  var exportBtn = document.getElementById("exportBtn");
  var data = new Map(JSON.parse(localStorage.getItem("data")));
  var attachBtn = (name, link) => {
    ...
    eraseBtn.addEventListener("click", () => eraseBtn.parentElement.remove());
    container.append(anchor, eraseBtn);
    bookmarksList.appendChild(container);
  };
  var update = (name, link) => {
    data.has(name) ? anchors[[...data].findIndex((k) => k[0] === name)].href = link : attachBtn(name, link);
    data.set(name, link);
    saveData(data);
  };
  updateBtn.addEventListener("click", () => {
    const [name, link] = prompt(
      "Add or Edit an exisiting bookmark by entering Name & URL separated by comma"
    ).split(",");
    if (link)
      update(name, link);
  });
  ...
  exportBtn.addEventListener("click", () => {
    if (data.size) {
      ...
      link.href = URL.createObjectURL(
        new Blob([localStorage.getItem("data")], { type: "application/json" })
      );
